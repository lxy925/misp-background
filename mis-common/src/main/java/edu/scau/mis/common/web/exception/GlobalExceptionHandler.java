...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResult<?>> handleMethodArgumentTypeMismatchException(NoResourceFoundException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
    ...
    public ResponseEntity<ApiResult<?>> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
    ...
    public ResponseEntity<ApiResult<?>> handleServiceException(ServiceException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        log.error("请求地址:'{}',发生业务逻辑异常:'{}'", requestURI, e.getMessage());
        return ResponseEntity.status(HttpCode.SERVICE_LOGIC_EXCEPTION.getCode()).body(ApiResult.fail(e.getCode(),e.getMessage()));
    }
}
